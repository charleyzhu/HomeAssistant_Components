...
class HueDingDongConfigView(HomeAssistantView):
    """Handle Support DingDong Smart SoundBox"""

    url = '/api/{username}'
    name = 'emulated_hue:Config:state'
    requires_auth = False
    ...
    def get(self ,request,username):
        if username == "null":
            return self.json([{"error":{"type":"1","address":"null","description":"unauthorized user"}}])
        else:
            ...
            for entity in hass.states.async_all():
                ...
                if self.config.is_entity_exposed(entity):
                    ...
                    light = entity_to_json(self.config, entity, state, brightness)
                    light["manufacturername"] = "philips"
                    light["state"]["alert"] = "none"
                    lights[number] = light

            json_response = {}
            json_response["lights"] = lights
            json_response["scenes"] = {}
            ...
            config["gateway"] = util.get_local_ip()
            config["mac"] = get_mac_address()
            config["swversion"] = "9999999999"
            config["apiversion"] = "1.19.0"
            config["linkbutton"] = True
            config["ipaddress"] = util.get_local_ip()
            config["proxyport"] = 0
            config["portalservices"] = False
            config["swupdate"] = swupdate
            config["netmask"] = self.config.netmask
            config["dhcp"] = True
            config["utc"] = get_utc_time()
            config["proxyaddress"] = "none"
            config["localtime"] = get_local_time()
            config["timezone"] = self.config.timezone
            config["zigbeechannel"] = "6"
            config["modelid"] = "HomeAssistant"
            config["bridgeid"] = get_bridgeid()
            config["factorynew"] = False

            whitelist = {}
            whitelist_dingdong = {}
            whitelist_dingdong["createdate"] = get_local_time()
            whitelist_dingdong["lastusedate"] = get_local_time()
            whitelist_dingdong["name"] = "dingdong#allwinner-tablet#{0}".format(request._state.get("ha_real_ip"))

            whitelist["ab2633c3ed104604afc5709d4d9cac9e"] = whitelist_dingdong
